...
//                    $objActSheet->setCellValue($cellName[$j].($i+2), $data['data_list'][$i][$data['xls_cell'][$j][0]]);
   ...
   /* 新增方法，读取excel数据，并返回array
    *
    */
   public function read_excel_to_array($path){
     $objPHPExcel = new \PHPExcel();
     $objReader = $objPHPExcel->load($path); //需要在前面加反斜杠，因为命名空间
        
       /* $allWorksheets = $PHPReader->getAllSheets(); //获取所有工作表*/
        var_dump($objReader);
        /*$objPHPExcel = new \PHPExcel();
        $getExcelObject=$objPHPExcel->loadExcel($full_path);
