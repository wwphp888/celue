...
use app\vip\model\Withdraw as WithdrawModel;
use think\Db;
use think\Hook; 
...
class Withdraw extends Admin{
        ...
        if(isset($map['vip_name'])){
            $map['withdraw_vip'] = Db::name("vip")->where("vip_name",$map['vip_name'][1])->value('id');
            unset($map['vip_name']);
        }
        $data_list = WithdrawModel::with('vip')->where($map)->order($order)->paginate();
        foreach ($data_list as $key => $value) {
        	$value->vip_name = $value->vip->vip_name;
        	$value->vip_phone = $value->vip->vip_phone;
        }
        // 导出按钮 
        $btn_excel = [
            'title' => '导出Excel文件',
        ...
        return ZBuilder::make('table')
         ->setTableName('vip_withdraw') // 设置数据表名
         ->setPageTitle('提现列表')
         ->setSearchArea([['text', 'vip_name', '用户名'],])
         ->addColumns([ // 批量添加列
	        ['id', 'ID'],
	        ['vip_name','所属会员'],
	        ['vip_phone','手机号'],
		...
		public function audit(){
			 ...
			 if ($this->request->isGet()) {
			 		...
			 		if($type == 1){
			 			...
			 			$res = WithdrawModel::update($data);
			 			money_log($infos['withdraw_amount'],$infos['withdraw_vip'],3,'提现成功');
			 		}else{
			 			$data['withdraw_status'] = 2;
			 			$res = WithdrawModel::update($data);
			 			money_log($infos['withdraw_amount'],$infos['withdraw_vip'],15,'提现失败');
			 		}
