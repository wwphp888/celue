...
if (!function_exists("is_page_active")) {
    function is_page_active($menu){
        ...
        if($menu == '/' && $controller == 'Index'){
            $res = true;
        }elseif($controller != 'Index'){
            $res = stripos($menu,$controller);
        }
...
if(!function_exists("get_socket_info")){
    function get_socket_info($params){
        // 建立socket连接到内部推送端口
...
if(!function_exists("get_code_info")){
    function get_code_info($code){
          $rid = rand(1000,999999);
            $info =json_decode(get_socket_info('{"req":"Trade_QueryQuote","rid":"'.$rid.'","para":{"Codes" : "'.$code.'","JsonType" : 1,"Server" : 1}}'),true)['data'];
             //echo $info;
             return $info;
...
/*新浪股票数据
*/
if (!function_exists('sina_market_bs')) {
        function sina_market_bs($code)
        {
            $codes = fenxis($code);
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'http://hq.sinajs.cn/list=' . $codes);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_HEADER, 0);
            $output = curl_exec($ch);
            curl_close($ch);
            $tmd = explode(',', mb_convert_encoding($output, 'utf-8', 'gbk'));
            //echo "<pre>";
            $data = sina_to_api_bs($tmd,$code);
           
            return $data;
        }
    }
    if (!function_exists('sina_to_api_bs')) {
        function sina_to_api_bs($res,$code){
            
            $data[0] = '0'; //市场
            $data[1] = $code; //代码
            $data[2] = ''; //活跃度
            $data[3] = round($res[3],2); //现价
            $data[4] = $res[2]; //昨收
            $data[5] = $res[1]; //开盘
            $data[6] = $res[4]; //最高
            $data[7] = $res[5]; //最低
            ...
            $data[17] = $res[6]; //买一价
            $data[18] = $res[21]; //卖一价
            $data[19] = intval($res[10]/100); //买一量
            $data[20] = intval($res[20]/100); //卖一量
            $data[21] = $res[13]; //买二价
            $data[22] = $res[23]; //卖二价
            $data[23] = intval($res[12]/100); //买二量
            $data[24] = intval($res[22]/100); //卖二量
            $data[25] = $res[15]; //买三价
            $data[26] = $res[25]; //卖三价
            $data[27] = intval($res[14]/100); //买三量
            $data[28] = intval($res[24]/100); //卖三量
            $data[29] = $res[17]; //买四价
            $data[30] = $res[27]; //卖四价
            $data[31] = intval($res[16]/100); //买四量
            $data[32] = intval($res[26]/100); //卖四量
            $data[33] = $res[19]; //买五价
            $data[34] = $res[29]; //卖五价
            $data[35] = intval($res[18]/100); //买五量
            $data[36] = intval($res[28]/100); //卖五量
            $data[37] = ''; //保留4
            $data[38] = ''; //保留5
            $data[39] = ''; //保留6
            $data[40] = ''; //保留7
