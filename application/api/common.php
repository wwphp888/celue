...
/*if (PHP_SAPI != 'cli') {
	...
	if (count($dfu) <= 1) {
		$dfu = explode('hongtutueeha.cn', $defurl);
	}

	if (count($dfu) <= 1) {
		$dfu = explode('chengchengsl.cn', $defurl);
	}

	if (count($dfu) <= 1) {
		$dfu = explode('39.98.34.239', $defurl);
	}
	...
	if (count($dfu) <= 1) {
		...
		exit('<div style=\'height:200px;line_height:200px;font-size:24px;text-align:center;\'>对不起,该程序没有对您正在使用的域名进行授权,如需使用，请联系客服获取授权！</div>');
	}
}*/
