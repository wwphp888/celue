...
function getgupiaoinfos(){
	$.post("./getsocketinfo",{"stcode":stcode},function(res){
		priceData = res;
		var titybs;
		//基本行情
		if(res[3] == 0){
		...
		var money = $("#money").val();
		if(money < 1){
			min_money = Math.ceil(res[3]/2)*100;
			money = min_money;
			$("#money").val(min_money);
			var ying_moneys = parseInt(money)+parseInt(strategy_fee);
...
function searchStock(key){
	$.get("./searchStock",{"str":key},function(res){
...
function benjin_tjs(money){
	...
	var benjin_tj = '';
	//console.log(strategy_rate_list);
	var ying_moneys = parseInt(money)+parseInt(strategy_fee);
	$("#ying_money").html(ying_moneys);
	for (var i = 0; i < strategy_rate_list.length; i++) {
		strategy_rate = strategy_rate_list[0]; //赋值信用金倍率
		var quzheng = parseInt(money*strategy_rate_list[i]/priceData[3]/100);
		number = quzheng*100;
		var yings = parseInt(quzheng*priceData[3]*100)+parseInt(money);
...
$("#money").bind('blur',function(){
	var money = $("#money").val();
	min_money = Math.ceil(priceData[3]/2)*100;
	if(money < min_money){
		$("#money").val(min_money);
		benjin_tjs(min_money);
		layer.msg("最少信用金为"+min_money);
...
$('.stymoney1 li').click(function(){
		...
		number = parseInt(money*strategy_rate/priceData[3]/100)*100;
		//止盈价格
		//console.log(number);
		var winstop_price  = (parseFloat(priceData[3]*winstop/100)+parseFloat(priceData[3])).toFixed(2);
		//止损价格
...
layer.confirm('您确定要提交并委托该策略吗？', {
			  ...
			  //自动续费
				...
				min_money = Math.ceil(priceData[3]/strategy_rate)*100;
				 ...
				 $.post("./tradebuy",{"stname":stname,"stcode":stcode,"money":money,"strategy_rate":strategy_rate,"autostatus":autostatus,"winstops":winstops,"downstops":downstops,'number':number,'price':priceData[3]},function(res){
				 	console.log(res);
...
function stopwinedit(id){
	$.get("./getmatchorder",{"id":id},function(res){
		matchinfo = res;
		console.log(res);
...
function dostopwinedit(id){
		...
		$.get("./setstopstatus",{"id":id,"type":"1","val":add},function(res){
...
function dostopdownedit(id){
	...
	$.get("./setstopstatus",{"id":id,"type":"2","val":crd,"credit":credit_money},function(res){
...
//大赛持仓订单 
function getmatchorderlist(){
	$.get("./getcclist",{"status":1},function(res){
		var htmls='';
		for (var i =0; i < res.length; i++) {
			...
			htmls +='<div class="cell-right" id="wifi'+res[i].id+'">';
		...
		$("#cc_strategy_list").html(htmls);

	})
}/*setInterval("getmatchorderlist()",3000);*/
//修改递延费开启状态
function setswitch(id,status){
	layer.msg('设置中', {icon: 16,shade: 0.01,time:10000 });
	$.get("./setswitch",{"id":id,"status":status},function(res){
		layer.closeAll();
		if(res.status){
			layer.msg(res.message);
		}else{
...
function subsell(id){
	layer.confirm('您确定要提交委托卖出该持仓策略吗？', {
			  ...
			  title:'卖出策略'
				...
				$.get("./subsell",{"id":id},function(res){
					layer.closeAll();
					if(res.status){
						layer.msg(res.message);
					}else{
...
//大赛历史记录
//大赛持仓订单
function getlogorderlist(){
	$.get("./getcclist",{"status":2},function(res){
		var htmls=''; 
		for (var i =0; i < res.length; i++) {
			
			htmls += '<div class="positions"><div class="posmain"><dl>';
			htmls +='<dt>'+res[i].gupiao_name+'<span>'+res[i].gupiao_code+'</span></dt>';			
			htmls +='<dd>股票数量    <span>'+res[i].trush_number+'</span></dd></dl>';
			htmls +='<dl><dt>收益    <span class="red">'+res[i].repay_profits+'</span></dt>';
			htmls +='<dd>卖出类型    <span class="green">'+res[i].sell_types+'</span></dd></dl>';
			htmls +='<dl><dt>买入价    <span>'+res[i].trush_price+'</span></dt>';
			htmls +='<dd>卖出价    <span class="red">'+res[i].sell_price+'</span></dd></dl>';
			htmls +='<dl><dt>信用金<span>'+res[i].credit_money+'</span></dt>';
			htmls +='<dd>返还信用金    <span class="red">'+res[i].repay_creat_money+'</span></dd></dl>';
			htmls +='<dl><dt>买入时间    <span>'+res[i].create_times+'</span></dt>';
			htmls +='<dd>卖出时间      <span class="red">'+res[i].sell_times+'</span></dd></dl>';
			htmls +='<a class="sell" href="javascript:void(0)" onclick="checkinfo('+res[i].id+')">查看</a></div></div>';
